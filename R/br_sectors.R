...
br_sectors = function(sector_df, var_choice, ytitle, ylim) {
  
  df = sector_df %>% filter(variable == var_choice)
  
  ghgi = df %>% filter(type == "GHGI")
  ghgi_2021 = ghgi %>% filter(year == 2021)
  
  lts_no2021 = df %>% filter(type == "LTS")
  lts_2021 = lts_no2021 %>% 
    filter(year == 2025) %>%
    mutate(
      value = ghgi_2021$value,
      year = 2021,
      datasrc = "copied from GHGI")
  lts = rbind(lts_no2021, lts_2021)
