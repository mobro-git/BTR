...
gtsave(pct_change_html_gas, paste0('output/',settings$version,"/tables_figs_sens/results_tables/percent_change_2005/",filename))
```
...
gtsave(pct_change_html_sector, paste0('output/',settings$version,"/tables_figs_sens_sens/results_tables/percent_change_2005/",filename))
```
...
ggsave(paste0('output/',settings$version,"/tables_figs_sens/btr_main_figs/main_fig/",filename), create.dir = TRUE, height = 5.5)
```
...
ggsave(paste0('output/',settings$version,"/tables_figs_sens/btr_main_figs/pct_change/",filename), create.dir = TRUE, height = 6)
```
...
proj = rbind(proj_no_connect, proj_connect) %>%
  mutate(year = as.numeric(year)) %>%
  filter(grouping %in% c("IRA", 'wm_sens')) %>%
  mutate(grouping = case_when(
    grouping == "wm_sens"~"2024 Policy Baseline",
    grouping == "IRA" ~ "LEEP",
    TRUE~grouping))
...
ggsave(paste0('output/',settings$version,"/tables_figs_sens/btr_main_figs/main_fig_leep/",filename), create.dir = TRUE, height = 6)
```
...
```{r}
filename <- paste0("btr_main_fig_pctchange05_",Sys.Date(),".png")
ggsave(paste0('output/',settings$version,"/tables_figs_sens/btr_main_figs/main_fig_leep/",filename), create.dir = TRUE)
```
...
multipanel_kaya
ggsave(paste0('output/',settings$version,"/tables_figs_sens/kaya/kaya_multipanel.png"), width = 5, create.dir = TRUE)
...
ggsave(paste0('output/',settings$version,"/tables_figs_sens/kaya/",filename), create.dir = TRUE)
```
...
```{r}
buildings
ggsave(paste0('output/',settings$version,"/tables_figs_sens/sector_analysis/buildings_comparison.png"), create.dir = TRUE, height = 4.5)
```
...
```{r}
electricity
ggsave(paste0('output/',settings$version,"/tables_figs_sens/sector_analysis/electricity_comparison.png"), create.dir = TRUE, height = 4.5)
```
...
```{r}
transportation
ggsave(paste0('output/',settings$version,"/tables_figs_sens/sector_analysis/transportation_comparison.png"), height = 4.5)
```
...
```{r}
industry
ggsave(paste0('output/',settings$version,"/tables_figs_sens/sector_analysis/industry_comparison.png"), create.dir = TRUE, height = 4.5)
```
...
# Energy
nrg_ch4 <- projections_all_sm %>% filter(grouping == 'wm_sens',gas == 'CH4',usproj_sector == 'Energy',year %in% config$table) %>% select(year, sum, pct_change_05) %>% distinct()

nrg_co2 <- projections_all_sm %>% filter(grouping == 'wm_sens',gas == 'CH4',usproj_sector == 'Energy',year %in% config$table) %>% select(year, sum, pct_change_05) %>% distinct()
...
gen_subset_sector_df <- function(sector_df, var_choice) {
  sector_df %>%
    filter(variable == var_choice,
           region == 'United States',
           scenario %in% c('Historic','wm_sens'),
...
gtsave(ncbr_comp_table_longnames, paste0('output/',settings$version,"/tables_figs_sens/results_tables/ncbr_comparison/longnames/",filename))
```
...
```{r}
filename <- paste0("ncbr_comp_fig_",Sys.Date(),".png")
ggsave(paste0('output/',settings$version,"/tables_figs_sens/ncbr_comparison/longnames/",filename), create.dir = TRUE)
```
...
ggsave(paste0('output/',settings$version,"/tables_figs_sens/results_tables/ncbr_comparison/",filename))
```
