...
gtsave(ncbr_comp_table, paste0('output/',settings$version,"/tables_figs/results_tables/ncbr_comparison/longnames/",filename))
```
...
lulucf_fig <- ggplot(mapping = aes(x = year, y = value)) +
 ...
 # geom_point(data = lulucf_plot_data, color = color_id) +
  ...
  scale_x_continuous(breaks = c(2000, 2005, 2010, settings$base_year, 2025, 2030, 2035, 2040)) +
  expand_limits(y = c(0,-1500)) +
  scale_y_continuous(labels = scales::comma, breaks = c(0,-500,-1000,-1500), expand = c(0, 0)) +
  theme_btr()
lulucf_fig
...
table <- gt(df) %>% fmt_percent(columns = 2:10,
                         decimals = 0) %>%
...
nrgco2 = projections_all_sm %>%
  ...
  group_by(grouping,year,usproj_sector) %>%
  summarize(min = min(pct_change_05),
            max = max(pct_change_05))

# buildings - res and com % electricity of total demand
com_res_elecrif = data_long_clean %>%
  filter(scenario == "wm",
         variable %in% c("Final Energy|Buildings|Residential|Percent Electricity",
                         "Final Energy|Buildings|Commercial|Percent Electricity"),
         year == 2040)
```
