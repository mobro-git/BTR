...
tge_all_long <- total_gross_emissions %>%
  pivot_longer(cols = 3:10, names_to = 'year') %>%
  mutate(Projection = '2024 BTR') %>%
...
```{r}
fig <- ncbr_comparison_figure(ncbr_comp_data_hist, tge_all_long, settings)
fig
...
fig <- ncbr_comp_fig_1990(tge_90_long, tge_all_long, settings)
fig
