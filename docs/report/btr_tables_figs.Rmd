...
br_comp_df <- read_csv('data-raw/ncbr_comparison/total_gross_ghg_ncbr_comparisons.csv')


# gross_totals_df <- br_comp_df %>%
...
tge_long <- total_gross_emissions %>%
...
btr_gross_df <- tge_long_max %>% rbind(tge_long_min)


# Rbind past NCBRs with BTR
tge_rbind <- btr_gross_df %>%
  bind_rows(gross_totals_df) %>%
