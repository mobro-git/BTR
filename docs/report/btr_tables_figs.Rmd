...
lulucf_nfs <- read_xlsx("data-extra/USDA NFS Raw Data/LULUCF projections DRAFT compilation 82924.xlsx",
                        sheet = "LULUCF TABLE FOR PIPELINE") %>%
  filter(model == 'NFS') %>%
  pivot_longer(cols = 6:11, names_to = 'year') %>%
  select(year, value)

lulucf_plot_data <- lulucf_co2_hist_long %>% rbind(lulucf_nfs) %>%
  mutate(
    year = as.numeric(year),
    value = as.numeric(value),
    group = case_when(
      year <= settings$base_year ~ 'hist',
      year > settings$base_year ~ 'proj'
    )
  ) %>%
...
lulucf_fig <- ggplot(mapping = aes(x = year, y = value)) +
  geom_point(data = (lulucf_plot_data %>% filter(year >= 2022)), color = '#0097FF') +
  geom_line(data = lulucf_plot_data, color = '#0097FF') +
...
df <- data_long_clean %>%
  ...
  select(model, year, pct_change_05, value)
return(df)
}
...
table <- gt(df) %>% fmt_percent(columns = pct_change_05,
                         decimals = 0) %>%
  cols_label(pct_change_05 = "% Change from 2005") %>%
  cols_align(align = 'center',
             columns = pct_change_05)
return(table)
}
...
table <- gt(df) %>% fmt_percent(columns = value,
                         decimals = 0) %>%
  cols_label(value = "Electric Vehicle Sales Stock") %>%
  cols_align(align = 'center',
             columns = value)
return(table)
}
