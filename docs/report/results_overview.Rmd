...
get_pct_change_40_df <- function(data_long_clean, var_list, config){
  ...
  df <- data_long_clean %>%
    ...
    group_by(model) %>% 
    mutate(pct_change_40 = round(100*(1 - (value[year==2040]/value[year==2020])))) %>%
...
nrgco2_value_list <- get_pct_change_40_value_list(pct_change_40_df)

figure_subset_df <- get_figure_subset_df(figmap_nrgco2_timeseries,9)
var_list <- c("Emissions|CO2|Energy|Demand|Buildings")
pct_change_40_df <- get_pct_change_40_df(data_long_clean, var_list,config)
buildings_value_list <- get_pct_change_40_value_list(pct_change_40_df)

figure_subset_df <- get_figure_subset_df(figmap_nrgco2_timeseries,9)
var_list <- c("Emissions|CO2|Energy|Demand|Industry")
pct_change_40_df <- get_pct_change_40_df(data_long_clean, var_list,config)
industry_value_list <- get_pct_change_40_value_list(pct_change_40_df)

figure_subset_df <- get_figure_subset_df(figmap_nrgco2_timeseries,9)
var_list <- c("Emissions|CO2|Energy|Demand|Transportation")
pct_change_40_df <- get_pct_change_40_df(data_long_clean, var_list,config)
trn_value_list <- get_pct_change_40_value_list(pct_change_40_df)
