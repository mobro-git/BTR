---
pagetitle: "Tables and Figures for OAP "
output:
  html_document:
...
ggsave(paste0('output/',settings$version,"/tables_figs/btr_main_figs/",filename), create.dir = TRUE)
```
...
```{r}
buildings
ggsave(paste0('output/',settings$version,"/tables_figs/sector_analysis/buildings_comparison.png"), create.dir = TRUE)
```
...
```{r}
electricity
ggsave(paste0('output/',settings$version,"/tables_figs/sector_analysis/electricity_comparison.png"), create.dir = TRUE)
```
...
```{r}
industry
ggsave(paste0('output/',settings$version,"/tables_figs/sector_analysis/industry_comparison.png"), create.dir = TRUE)
```
...
tge_all_long_with_brvs = read_xlsx("data-extra/BR Voluntary Supplement 2023/totalgrossemissions.xlsx") %>%
...
```{r}
fig <- ncbr_comparison_figure(ncbr_comp_ribbon_with_brvs, tge_all_long_with_brvs, settings, config)
fig
```

```{r}
filename <- paste0("ncbr_comp_fig_with_brvs",Sys.Date(),".png")
ggsave(paste0('output/',settings$version,"/tables_figs/ncbr_comparison/",filename), create.dir = TRUE)
```
...
ggsave(paste0('output/',settings$version,"/tables_figs/nrg_emissions/",filename), create.dir = TRUE)
```
